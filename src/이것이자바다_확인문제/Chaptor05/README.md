...
- **정답: ➍**
**해설**
>참조 타입 변수는 객체를 참조하지 않는다는 의미로 `null` 값을 가질 수 있다. `null`로 초기화된 변수는 스택 영역에 생성되며, 어떤 객체도 참조하지 않는 상태를 나타낸다.
 ...
 **정답: ➌**
 **해설**
 >자바는 가비지 컬렉터(GC)가 참조되지 않는 객체를 자동으로 찾아 메모리에서 제거하므로, 프로그래머가 직접 객체를 소멸시키는 코드를 작성할 필요가 없으며 작성할 수도 없다.
 ...
 **정답: ➋**
**해설**
 >참조 타입인 String 객체의 내부 문자열 값을 비교할 때는 `==` 연산자가 아닌 `equals()` 메소드를 사용해야 한다. `==` 연산자는 객체의 주소값을 비교하기 때문에 문자열 내용이 같아도 `false`가 나올 수 있다.
 ...
 **정답: ➌**
**해설**
 >`boolean` 타입 배열 항목의 기본 초기값은 `true`가 아니라 `false`이다. 자바에서 배열을 생성할 때 별도의 값을 지정하지 않으면 각 타입의 기본값으로 자동 초기화된다.
